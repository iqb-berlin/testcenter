...
class WorkspaceValidator extends Workspace {
    ...
    private function readResources() {
        ...
        foreach ($this->allFiles['Resource'] as $path) {

            $this->allResources[] = FileName::normalize(basename($path), false);
            $this->resourceSizes[FileName::normalize(basename($path), false)] = filesize($path);
            $this->allVersionedResources[] = FileName::normalize(basename($path), true);
            $this->resourceSizes[FileName::normalize(basename($path), true)] = filesize($path);
        }
