...
.PHONY: run run-detached down start stop restart status logs config system-prune volumes-prune images-cleanvconnect-db \
	dump-all restore-all dump-db restore-db dump-db-data-only restore-db-data-only update
...
dump-all:
	docker exec testcenter-db mysqldump --verbose --all-databases --add-drop-database --user=root\
		--password=$(MYSQL_ROOT_PASSWORD) >backup/database_dump/all-databases.sql
...
restore-all:
	sed -i 's/\/\*!40000 DROP DATABASE IF EXISTS `mysql`\*\/;/ /g' backup/database_dump/all-databases.sql
	docker exec -i testcenter-db mysql --verbose --user=root --password=$(MYSQL_ROOT_PASSWORD)\
		<backup/database_dump/all-databases.sql

## Extract a database into a sql format file
# (https://dev.mysql.com/doc/refman/8.0/en/mysqldump-sql-format.html)
dump-db:
	docker exec testcenter-db mysqldump --verbose --add-drop-database --user=$(MYSQL_USER)\
		--password=$(MYSQL_PASSWORD) --databases $(MYSQL_DATABASE) >backup/database_dump/$(MYSQL_DATABASE).sql
...
restore-db:
	docker exec -i testcenter-db mysql --verbose --user=$(MYSQL_USER) --password=$(MYSQL_PASSWORD)\
		<backup/database_dump/$(MYSQL_DATABASE).sql
...
dump-db-data-only:
	docker exec testcenter-db mysqldump --verbose --no-create-info --user=$(MYSQL_USER)\
		--password=$(MYSQL_PASSWORD) --databases $(MYSQL_DATABASE) >backup/database_dump/$(MYSQL_DATABASE)-data.sql

## Restore a database data from a sql format file
# (https://dev.mysql.com/doc/refman/8.0/en/reloading-sql-format-dumps.html)
restore-db-data-only:
	docker exec -i testcenter-db mysql --verbose --user=$(MYSQL_USER) --password=$(MYSQL_PASSWORD)\
		<backup/database_dump/$(MYSQL_DATABASE)-data.sql
