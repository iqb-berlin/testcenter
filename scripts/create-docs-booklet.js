...
const renderRef = ref => {
  ...
  // Unser zentrales Wörterbuch, das die technischen Namen in schöne Titel übersetzt
  const inlineMapping = {
  ...
  return ` → Siehe Abschnitt: **${cleanName}**`;
};
...
const renderDefs = (schema, current) => {
  ...
  Object.keys(schema.$defs || {}).forEach(defName => {
    ...
    // Keine ID-Anker mehr nötig! Direkt die saubere Überschrift ohne Backticks:
    result += `\n## ${cleanName}\n\n`;

    if (def.description) result += `${def.description}\n\n`;

    if (def.properties) {
      // Für den Parameter-Pfad (z.B. Testlet.id) schneiden wir den langen Namen ab
...
// ---------------------------------------------------------------------------
// Gulp-Task: NEU - Generiert die komplett eigenständige adaptive-config.md
// ---------------------------------------------------------------------------
exports.adaptiveConfig = done => {
  ...
  Object.keys(adaptiveSchema.$defs || {}).forEach(defName => {
    ...
    // Auch hier: Keine ID-Anker mehr nötig, einfach nur die Überschrift schreiben!
    output += `\n## ${cleanName}\n`;
