          ...
          if (file_exists($testTakerFilesFolder)) {
            ...
            if ($ttDir !== false) {
              ...
              foreach($myDBConnection->getBookletsStarted($wsId) as $b) {
                ...
                $tmpTime = strtotime($b['laststart']);
                if ($tmpTime > $keyedReturn[$b['groupname']]['laststart']) {
                  $keyedReturn[$b['groupname']]['laststart'] = $tmpTime;
                  $keyedReturn[$b['groupname']]['laststartStr'] = strftime('%d.%m.%Y',$tmpTime);
                }
              }
            }
