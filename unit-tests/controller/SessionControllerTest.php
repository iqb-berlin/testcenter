...
final class SessionControllerTest extends TestCase {
    ...
    private function addMockFunctions(MockObject $mockObject, array $functionsAndResults, array $expectFunctionCalls = []) {
        ...
        foreach($functionsAndResults as $function => $result) {
            ...
            if ($result instanceof Exception) {
                ...
                $mockObject->method($function)->willThrowException($result);
    ...
    public function test_putSessionLogin_registerGroupIfMonitor(): void {
        ...
        $testeeLogin = new Login(
            'testee_login',
            'password_hash',
            'run-hot-return',
            'sample_group',
            'Sample Group',
            [ 'testee1' => ['THE_BOOKLET'], 'testee2' => ['THE_BOOKLET']],
            1
        );
        ...
        $loginSessionTestee = new LoginSession(
            1,
            'some_token',
            new Login(
                'test',
        ...
        $this->mockSessionDAO(
            [
                ...
                'createLoginSession' => $loginSessionTestee,
                'getOrCreatePersonSession' => new PersonSession(
                    $loginSessionTestee,
                    new Person(1, 'person_token', '')
                ),
                'getLoginsByGroup' => [
                    new LoginSession(2, 'token2', $testeeLogin),
                    new LoginSession(3, '', $testeeLogin),
                    new LoginSession(4, '', $testeeLogin),
                    new LoginSession(5, '', $testeeLogin),
                ]
            ],
            [
                'getOrCreateLoginSession' => 1,

                'getOrCreatePersonSession' => 5, // 4 persons
                'getLoginsByGroup' => 1, // 1
            ]
        );

        $this->mockTestDAO([
            'getOrCreateTest' => ['id' => 99],
        ]);
